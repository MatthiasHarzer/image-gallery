<script lang="ts">
  ...
  function fireScale(touchA, touchB) {
    if (!zoomEnabled) return;
    const xTouch = [
      Math.min(touchA.pageX, touchB.pageX),
      Math.max(touchA.pageX, touchB.pageX),
    ];

    const yTouch = [
      Math.min(touchA.pageY, touchB.pageY),
      Math.max(touchA.pageY, touchB.pageY),
    ];
  ...
  function fireTapScale(x, y) {
    ...
    let in_x =
      (window.innerWidth - ratio.width * Math.max(xFactor * scaleVtm, 1)) / 2;
    let in_y =
      (window.innerHeight - ratio.height * Math.max(xFactor * scaleVtm, 1)) / 2;
  ...
  function onLoad() {
    ...
    contain =
      naturalWidth > window.innerWidth || naturalHeight > window.innerHeight;
  ...
  function onTouchStart(e) {
    ...
    if (isMultiTouch) {
      ...
      if (
        ...
        Math.hypot(lastTap.x - pageX, lastTap.y - pageY) <= 20
      ) {
        smooth = true;
