<script lang="ts">
  ...
  $: renderedImages = [
    previousImagePromise,
    imagePromise,
    upcomingImagePromise,
  ];
  $: loaded =
    renderedImages && renderedImages.every((promise) => promise != null);
...
<div class="main">
  <div
    ...
    class="scrollable-image-wrapper"
  ...
  <FullscreenViewNav
    ...
    on:close={onClose}
    {zooming}
    navShown={$localConfig.showFullscreenNav}
    on:delete={onDelete}
    on:toggle-nav={toggleNav}
  />
</div>
