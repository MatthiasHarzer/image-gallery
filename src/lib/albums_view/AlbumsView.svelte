<script lang="ts">
  ...
  $: selectedAlbum =
    $route.albums.length > 0 ? $route.albums[$route.albums.length - 1] : null;
  ...
  const deleteAlbum = async ({
    detail: album,
  }: CustomEvent<Album>): Promise<void> => {
...
<div class="main">
  <div class="header-nav-bar">
    ...
    <button
      class:hidden={!openedAlbum?.valid && !openedAlbum?.isRoot}
      class="material text-button add-album"
